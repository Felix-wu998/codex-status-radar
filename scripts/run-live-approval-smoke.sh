...
APP_LOG="${CODEX_STATUS_RADAR_APP_LOG:-/tmp/codex-status-radar-app.log}"
...
rm -f "${APP_LOG}"
CODEX_STATUS_RADAR_DEBUG_LOG=1 \
  "${ROOT_DIR}/scripts/run-macos-app.sh" --foreground >"${APP_LOG}" 2>&1 &
started_app_pid="$!"
sleep 1

CODEX_APP_SERVER_PORT="${PORT}" \
CODEX_SPIKE_APPROVAL_RESPONSE_DELAY_MS="${CODEX_SPIKE_APPROVAL_RESPONSE_DELAY_MS:-3000}" \
node "${ROOT_DIR}/prototypes/app-server-approval/app-server-approval-spike.mjs"

if ! grep -q "waiting approval observed" "${APP_LOG}"; then
  echo "Codex Status Radar App 未记录到 waiting approval 状态，日志：${APP_LOG}" >&2
  exit 1
fi

echo "Codex Status Radar App 已记录到 waiting approval 状态。日志：${APP_LOG}"
