...
5. 灵动岛 UI 可以在没有审批请求体时展开提醒，在有审批请求体时展示原生审批按钮。
6. 旁路 observer 模式已验证能稳定收到 `waitingOnApproval`；真实 `requestApproval` 三按钮请求未证明会广播给旁路客户端，详见 `docs/research/approval-request-routing-spike.md`。
