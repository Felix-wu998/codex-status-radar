...
Codex Status Radar 是一个本地优先的 macOS Codex 伴随工具。它不替代 Codex，也不接管 Codex 的主流程。它观察本地 Codex app-server 状态，在灵动岛区域展示状态灯，并在 Codex 等待审批时弹出审批交互。
...
apps/macos
  ...
  NotchStatusController       管理灵动岛状态灯
...
8. 灵动岛审批 UI 按 `availableDecisions` 顺序展示按钮。
...
- 手动验证：菜单栏图标、灵动岛窗口位置、审批弹窗、点击后的状态切换。
- 后续稳定后再补 UI 自动化。
